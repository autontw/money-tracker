import streamlit as st
import pandas as pd
import os
from datetime import date
import plotly.express as px

# 1. ตั้งค่าหน้าเว็บ
st.set_page_config(
    page_title="ระบบบัญชีครูณัฐวัฒน์",
    page_icon="🐌",
    layout="wide"
)

# 2. ฟังก์ชันโหลดและบันทึกข้อมูล
FILE_NAME = "data.csv"

def load_data():
    if os.path.exists(FILE_NAME):
        return pd.read_csv(FILE_NAME)
    else:
        return pd.DataFrame(columns=["Date", "Item", "Category", "Type", "Amount"])

# ฟังก์ชันสำหรับเพิ่มข้อมูลใหม่ (จาก Sidebar)
def append_data(date_in, item, category, type_in, amount):
    df = load_data()
    new_data = pd.DataFrame({
        "Date": [date_in],
        "Item": [item],
        "Category": [category],
        "Type": [type_in],
        "Amount": [amount]
    })
    df = pd.concat([df, new_data], ignore_index=True)
    df.to_csv(FILE_NAME, index=False)
    return df

# ฟังก์ชันสำหรับบันทึกตารางที่แก้ไขแล้ว (Save ทับทั้งหมด)
def save_edited_data(df):
    df.to_csv(FILE_NAME, index=False)

# --- ส่วนของการแสดงผล (Frontend) ---

st.title("💰 My Income & Expense Tracker")

# 3. Sidebar: สำหรับกรอกข้อมูลใหม่
st.sidebar.header("📝 บันทึกรายการใหม่")
with st.sidebar.form("entry_form", clear_on_submit=True):
    date_input = st.date_input("วันที่", date.today())
    item_input = st.text_input("รายการ (เช่น ค่ากาแฟ)")
    type_input = st.selectbox("ประเภท", ["รายจ่าย", "รายรับ"])
    
    # หมวดหมู่ (ปรับให้เข้ากับครูและเจ้าของฟาร์ม)
    if type_input == "รายจ่าย":
        cat_options = ["อาหาร/เครื่องดื่ม", "เดินทาง/น้ำมัน", "ค่าอาหารหอย/ดูแลฟาร์ม", "อุปกรณ์การสอน/สื่อ", "ค่าเซิร์ฟเวอร์/เกม Unity", "บิล/สาธารณูปโภค", "อื่นๆ"]
    else:
        cat_options = ["เงินเดือนครู", "รายได้จากฟาร์ม", "สอนพิเศษ", "อื่นๆ"]
        
    category_input = st.selectbox("หมวดหมู่", cat_options)
    amount_input = st.number_input("จำนวนเงิน", min_value=0.0, format="%.2f")
    
    submitted = st.form_submit_button("บันทึกข้อมูล")
    
    if submitted:
        append_data(date_input, item_input, category_input, type_input, amount_input)
        st.sidebar.success("บันทึกสำเร็จ!")
        st.rerun() # รีเฟรชหน้าเว็บทันที

# 4. Main Dashboard
df = load_data()

if not df.empty:
    # --- ส่วนสรุปตัวเลข ---
    st.subheader("📊 ภาพรวมการเงิน")
    total_income = df[df["Type"] == "รายรับ"]["Amount"].sum()
    total_expense = df[df["Type"] == "รายจ่าย"]["Amount"].sum()
    balance = total_income - total_expense
    
    col1, col2, col3 = st.columns(3)
    col1.metric("รายรับรวม", f"฿{total_income:,.2f}", delta_color="normal")
    col2.metric("รายจ่ายรวม", f"฿{total_expense:,.2f}", delta="-inverse")
    col3.metric("เงินคงเหลือ", f"฿{balance:,.2f}")
    
    st.markdown("---")

    # --- ส่วนกราฟ ---
    c1, c2 = st.columns(2)
    with c1:
        st.write("### 🍰 สัดส่วนรายจ่าย")
        expense_df = df[df["Type"] == "รายจ่าย"]
        if not expense_df.empty:
            fig_pie = px.pie(expense_df, values='Amount', names='Category', hole=0.4)
            st.plotly_chart(fig_pie, use_container_width=True)

    with c2:
        st.write("### 📈 แนวโน้มรายรับ-รายจ่าย")
        df['Date'] = pd.to_datetime(df['Date'])
        df_grouped = df.groupby(['Date', 'Type'])['Amount'].sum().reset_index()
        if not df_grouped.empty:
            fig_bar = px.bar(df_grouped, x='Date', y='Amount', color='Type', barmode='group')
            st.plotly_chart(fig_bar, use_container_width=True)

    # --- ส่วนตารางแก้ไขข้อมูล (NEW!) ---
    st.subheader("📝 ประวัติรายการ (แก้ไข/ลบได้)")
    st.info("💡 **วิธีใช้:** ดับเบิ้ลคลิกเพื่อแก้ไขตัวเลข, หรือคลิกที่ช่องหน้าสุดของแถวแล้วกดปุ่ม Del บนคีย์บอร์ดเพื่อลบรายการ")

    # ใช้ st.data_editor แทน st.dataframe
    edited_df = st.data_editor(
        df, 
        num_rows="dynamic", # อนุญาตให้เพิ่ม/ลบแถวได้
        use_container_width=True,
        key="editor"
    )

    # ปุ่มบันทึกการแก้ไข
    if st.button("💾 บันทึกการแก้ไขทั้งหมด"):
        save_edited_data(edited_df)
        st.success("อัปเดตข้อมูลเรียบร้อย!")
        st.rerun()

else:
    st.info("ยังไม่มีข้อมูลในระบบ ลองกรอกข้อมูลทางเมนูด้านซ้ายดูสิครับ!")